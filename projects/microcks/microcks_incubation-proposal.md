...
Katie Gamanji conducted the due diligence of Microcks who applied for Incubation. The project has completed the criteria that show its maturity at Incubation. The following criteria implementations are noteworthy to call out:
- Microcks has an outstanding  list of adopters [ADOPTERS.md](https://github.com/microcks/.github/blob/main/ADOPTERS.md), with large-scale production usage, protocol versatility (consistently cited across adopters as a key differentiator, covering OpenAPI, AsyncAPI, GraphQL, and gRPC) and
Copilot feature emerging as a significant use case for LLM-generated synthetic data from API contracts.
- The project has also made significant efforts to enhance the overall security and compliance across all 19 repositories using [CLOMonitor checks](https://clomonitor.io/projects/cncf/microcks)
- Participated in LFX Mentorship Program 2025 with 7 projects and 7 mentees
- Broad ecosystem integrations: Helm, Kubernetes Operator, Prometheus, Grafana, OpenTelemetry, Backstage, Tekton, Jenkins, GitHub Actions, Testcontainers, Docker, and more


The following actions were provided to the project that were considered blocking but since resolved:

- [Complete] Onboard at least 1 maintainer to ensure the voting clause on project maintainers and deliverables is fulfilled. The other option is to update the voting guidelines.
- [Complete] For the main project, update the list of maintainers to include active contributors and develop a plan for onboarding new members.
- [Complete] For subprojects, identify reasons for stalled contributions and encourage activity or implement archiving initiatives as needed.
- [Complete] Clearly outline the project goals and objectives and/or out-of-scope items. This can be solved by completing the GTR or separately in the project documentation. Similar example already available as part of the security [self-assessment](https://github.com/yada/tag-security/blob/main/community/assessments/projects/microcks/self-assessment.md#goals).
- [Complete] Provide clear guidelines on how to cut/deploy a release, including cadence, versioning, etc., under a Release.md or similar file.
...
The adopter interviews reflect a project in use for the level which the project applied. Here are a few notable highlights:
- A large banking organisation integrated Microcks as the default mocking engine behind their internal API developer portal, enabling developers to discover and interact with over 250 APIs (targeting 400) through a single unified mocking engine rather than maintaining individual mock components per API.
- An IT consultancy deployed Microcks in production to support a Keycloak extension serving millions of end users in the retail sector, using Testcontainers integration within Azure DevOps CI/CD pipelines to enable automated integration testing where no external test environment was available.
- An organisation adopted Microcks specifically to reduce dependency on Mainframe environments during performance testing of microservices, using API-first mocking to decouple teams and minimise costly Mainframe usage.
- Multiple adopters highlighted the Copilot feature as a significant emerging use case, using LLM capabilities to automatically generate meaningful synthetic data from API contracts, solving a long-standing resourcing problem around the creation and maintenance of mock response data at scale.
- Across adopters, Microcks was consistently used as the sole mocking solution across multiple API standards simultaneously, covering OpenAPI, AsyncAPI, GraphQL, and gRPC within a single deployment, replacing fragmented or vendor-tied alternatives.
...
- Added a third Microcks maintainer from AXA (public adopter company)
  - Election vote: https://github.com/microcks/community/issues/115
  - Onboarding: https://github.com/microcks/community/issues/123
  - Updated `MAINTAINERS.md`: https://github.com/microcks/.github/blob/main/MAINTAINERS.md
  - Also listed in the CNCF maintainers list (see line 1634):
    https://github.com/cncf/foundation/blob/main/project-maintainers.csv
- Updated list of maintainers: all contributors are already visible on [GitHub](https://github.com/microcks/microcks/graphs/contributors).
Maintainers are planning to adopt the [all-contributors bot](https://allcontributors.org) to better recognize all types of contributions, improve visibility, and add a gamification aspect. They would also like to provide digital badges for their community and use [Holopin](https://www.holopin.io/).
- (optional) Steering Committee Established:
  - [Issue](https://github.com/microcks/community/issues/114)
  - [Charter](https://github.com/microcks/community/blob/main/steering/STEERING.md)
  - Also listed in the CNCF maintainers list - new ["Steering Committee" section](https://github.com/cncf/foundation/blob/main/project-maintainers.csv) (see line 1635).
  - [Initial members](https://github.com/microcks/community/issues/116)
  - [First meeting minutes](https://github.com/microcks/community/blob/main/steering/2026-03-19-minutes.md) and [recording](https://www.youtube.com/live/R-39A8X_zkQ?si=Y4s-CG_mGcdZphvA)
  - A dedicated Discord channel has also been created for Steering Committee discussions.
  - Ongoing applications for [Amadeus](https://github.com/microcks/community/issues/119) and [BNP](https://github.com/microcks/community/issues/121).
...
- (optional) list all available subprojects with their respective code owners (if applicable). This will also help the discoverability of ongoing initiatives related to Microcks.
  - Maintainer's response: This is expected as the maintainers file lists code owners and subprojects. All other repositories are managed by the core maintainers, and project maintainers are actively looking for additional code owners to help maintain these subprojects.

**Completed action items:**
- Stalled subprojects:
  - One notable example is [Hugo](https://github.com/microcks/microcks-docker-desktop-extension), which is directly linked to reduced activity on that specific subproject. Maintainers have initiated discussions to:
    - Restart activity via a mentees (mentorship program), or
    - Potentially archive the subproject.
    - Replace the code owner (Apoorva has been approached),
    - At this stage, the project remains functional and includes an auto-update mechanism to use the latest Microcks images on Docker Desktop.
...
**Completed action items:**
- Added a dedicated page presenting project goals:
  - [Issue](https://github.com/microcks/microcks.io/issues/516)
  - [Live page](https://microcks.io/documentation/overview/project-goals/)
- Completed the [GTR assessment](https://github.com/microcks/community/blob/main/cncf/technical-review/microcks-general-technical-questions.md)
...
- Provide timelines for milestone completion
  - Maintainer's response is that they agree with the lack of milestones. While they define release targets, maintainers do not commit to strict deadlines to allow time for community contributions. They are reviewing how other projects address this and will look for improvements.
...
**Completed action items:**
- Defined a new release process:
  - [Issue](https://github.com/microcks/microcks/issues/1954)
  - [PR](https://github.com/microcks/microcks/pull/1976)
  - [Documentation](https://github.com/microcks/microcks/blob/master/RELEASE.md)
- The next Microcks release (`1.14`) will follow this new process:
  https://github.com/microcks/microcks/issues/2009
- Improved [branch and release strategy](https://github.com/microcks/microcks/issues/1901)
...
<!-- (TOC Evaluation goes here) -->
A diagram of available integrations are available in the [Microcks update deck](https://docs.google.com/presentation/d/1dQpmqwOKFroAHPzhAzSXip1AnM96WUH-9UrVNiBgtP0/edit#slide=id.g32b6d8da232_0_279), which was also presented to TAG App Delivery.
