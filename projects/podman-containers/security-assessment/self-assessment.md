...
It runs containers on Linux, but can also be used on Mac and Windows systems using a Podman-managed virtual machine. It is based on libpod,
a library that provides APIs for container lifecycle management that is also contained within the Podman repository.

Technical characteristics:

- **Daemonless architecture**: Each command runs as a separate process
- **Rootless mode**: Uses user namespaces to run containers as non-root users
- **Pod support**: Implements Kubernetes-style pod semantics for grouping containers
- **OCI compliance**: Adheres to OCI runtime and image specifications
- **REST API**: Exposes Podman functionality via `podman system service`
- **Multi platform support**: Runs on Windows and Mac via virtual machines by `podman machine`

Podman is frequently used in the following scenarios:

- **Local development**: Running and testing containers on developer workstations.
- **Rootless container hosting**: Enabling non-privileged users to run containers on shared systems. Each user manages their own containers in their user namespace without access to other users' containers.
- **System administration**: Running containerized system services on Linux servers. Administrators use Podman with systemd unit files to manage long-running containerized services.

Podman is part of the containers ecosystem and depends on common libraries shared with related tools including Buildah (image building), Skopeo (image operations), and CRI-O (Kubernetes container runtime).
...
* **libpod library**: The core library that provides container lifecycle management APIs. It handles container creation, execution, and management. Directly used only by Podman.

* **REST API**: Exposes Podman functionality via `podman system service`.

* **Container runtime**: Interfaces with OCI-compliant runtimes (runc, crun) to actually run containers.

* **Image store**: Manages container images and their metadata. Images are stored in a local registry and are verified during the image pull process.
...
Rootless mode is subject to certain limitations as [detailed upstream](https://github.com/containers/podman/blob/main/rootless.md).
...
* **Container creation**:
  ...
  - Default security policies (seccomp, SELinux, capabilities),
  user-configurable via the `--security-opt` CLI option.
...
* **Code Review Process**: All code changes require review by two maintainers before merging. Critical security changes may require multiple reviews. The project uses GitHub pull requests for all contributions.
...
* **Buildah**: A command line tool to facilitate building OCI container images.

* **Podman Desktop**: A graphical tool for developing on containers and Kubernetes.

* **Skopeo**: A command line tool to perform various operations on container images and image repositories like copying an image, inspecting a remote image, deleting an image from an image repository.
