...
# CNCF TOC - تعریف پروژه

## تعریف

کمیته نظارت فنی CNCF (TOC) فرآیند و معیارهای درج پروژه‌های جدید در CNCF را تعریف می‌کند. TOC تضمین می‌کند که پروژه‌ها با استانداردهای CNCF مطابقت دارند و با مأموریت آن همسو هستند.

## معیارها

1. **مجوز متن باز**: پروژه‌ها باید تحت یک مجوز متن باز باشند.
2. **پذیرش**: پروژه‌ها باید یک پایگاه کاربری قابل توجه و جامعه فعال را نشان دهند.
3. **همسویی**: پروژه‌ها باید با اهداف و ارزش‌های CNCF همسو باشند.
4. **کد رفتار**: پروژه‌ها باید به کد رفتار CNCF پایبند باشند.
5. **حاکمیت**: پروژه‌ها باید یک مدل حاکمیت واضح و نگهدارنده‌ها داشته باشند.

## فرآیند

1. **ارسال پیشنهاد**: علاقه‌مندان یک پیشنهاد پروژه به TOC ارسال می‌کنند.
2. **بررسی TOC**: TOC پیشنهاد را بر اساس معیارها بررسی می‌کند.
3. **بازخورد جامعه**: پیشنهاد برای بازخورد جامعه باز می‌شود.
4. **تصمیم TOC**: TOC بر اساس بررسی و بازخورد تصمیم‌گیری می‌کند.

## نتیجه‌گیری

TOC CNCF متعهد به پرورش یک اکوسیستم متنوع و پایدار از فناوری‌های بومی ابری است.
