...
클라우드 네이티브 기술은 조직이 퍼블릭, 프라이빗, 그리고 하이브리드 클라우드와 같은 최신의 동적인 환경에서 확장성 있는 애플리케이션을 구축하고 운영할 수 있도록 지원해주며, 컨테이너, 서비스 메시, 마이크로서비스, 불변의 인프라스트럭처, 그리고 선언적 API 기술 등이 이에 해당한다.
 
이러한 클라우드 네이티브 기술들은 탄력적인 운영, 관리 및 진단 등을 제공하는 유연한 시스템을 가능하게 한다. 또한, 견고한 오토메이션을 함께 활용하면, 엔지니어는 영향력이 큰 시스템 변경 작업을 최소한의 노력으로 빈번하고 예측 가능하게 수행할 수 있다.
 
클라우드 네이티브 컴퓨팅 재단은 벤더 중립적인 오픈소스 프로젝트의 생태계를 육성하고 유지함으로써 클라우드 네이티브 패러다임의 활용 및 확산을 촉진하고, 관련된 최신 기술 및 패턴 등을 대중화하여 모든 사람들이 혁신적인 기술들을 쉽게 활용할 수 있게 한다.
