...
This compact profile captures the core idea. The workload needs a PostgreSQL database and expects the environment variables `DB_HOST`, `DB_PORT`, `DB_DATABASE`, `DB_USERNAME`, and `DB_PASSWORD` to be defined in order to establish its database connection. The profile declares that demand. The platform decides how to fulfill it.
